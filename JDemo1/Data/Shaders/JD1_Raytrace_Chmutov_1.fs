...
vec4 trace( vec3 xPos, vec3 xDir, const bool bDiscard )
{
	...
	// xNormal += 0.05 * normalize( sin( 120.0 * xSolution ) );
	...
	float fRipple1 = pow( 1.0 - ( -xRippleTimers.x + fRippleParameter ), 8 );
	vec4 xDiffuseTexture = vec4( fRipple1, fRipple1, fRipple1, 1.0 );
	/*
		texture2D( xTexture,
			vec2(
